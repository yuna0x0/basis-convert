...
`integrationType` and the PhysBone `version` field. Whatever the converter produces, the user
will open the resulting `JiggleRig` and tune it by feel. A build-time pass would regenerate
from source every build and throw that tuning away.
...
- Re-running must update rather than duplicate, so the converter needs bookkeeping tying each
  produced rig back to the PhysBone it came from.
- That bookkeeping cannot be a normal runtime component. See
  [0004](0004-no-runtime-components-on-the-avatar.md).
