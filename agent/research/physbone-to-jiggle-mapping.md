...
So a per-bone falloff curve carries across untouched rather than being flattened to a scalar.
...
term is zero under Simplified integration because it does not exist there. Pull is the force
returning bones to the rest pose, which is the closest thing PhysBone has to what jiggle calls
stiffness. Defaults 1.0 and 0.5.
...
would be implausibly heavy. Mapping onto a band instead puts a default PhysBone at drag 0.49,
next to the Hair preset.
